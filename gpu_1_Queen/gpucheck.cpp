...
int main(int argc, const char *argv[]) 
{
	...
	for(int i=0 ;i<rows_already_read; i++){
		...
		//showArr(a[i], len[i]);
		float sum_milli= 0 , sum_micro = 0,  sum_conflicts = 0;
		for(int t =0; t<replication; t++){
			c11_start =std::chrono::system_clock::now();
			int conflicts = get_conflicts( a[i], len[i]);
			...
			printf(	"\e[1;31;40m[%2d]  %10d\t%10d\t%10d\t%10d\t\n\e[0m", t , len[i], conflicts, milli_seconds, micro_seconds);
