...
__device__  void getPairs(
	int N, 						// Queen number;
	unsigned long long int x, 	// position in pairs[];
	int *Xi, 					// return i;
	int *Xj)					// return j;
{
	...
	int *Xj)					// return j;
{
	...
	int iRound=N-1;					// pairs of current round
	for(int i =0 ; i < N-1; i ++ ){ // N-1 round totally , i is first index  (i, ?)
		if(pos - iRound<= 0){
			*Xi=i;
			*Xj=i+pos;
			break;
		}
		pos -= iRound;				// index beyong current round [i]
		iRound--;    				// next round paris sub by 1
	}
}
