...
{
	/* printf("\n----get_conflicts() begin! \n"); */
	unsigned int *h_combination0= 0;//分配页锁定内存, 再把数据复制进来
	...
	checkCudaErrors(cudaMallocHost((void **)&h_combination0,  combination_size * sizeof(unsigned)));  //
	...
	checkCudaErrors(cudaMallocHost((void **)&h_combination1,  combination_size * sizeof(unsigned)));  //
...
// -----------------GPU0----------------------------------------------------------------------------
	checkCudaErrors(cudaSetDevice(0));
	checkCudaErrors(cudaMemcpyAsync(d_combination0, h_combination0, combination_size * sizeof( unsigned int), cudaMemcpyHostToDevice, stream0));
...
// -----------------GPU1----------------------------------------------------------------------------
	checkCudaErrors(cudaSetDevice(1));
	checkCudaErrors(cudaMemcpyAsync(d_combination1, h_combination1, combination_size * sizeof( unsigned int), cudaMemcpyHostToDevice, stream1));
	/* printf( "Switch to device : %d\n", getDevice()); */
	cuda_err= cudaSuccess;
