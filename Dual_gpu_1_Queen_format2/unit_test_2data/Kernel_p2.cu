...
// use 1 GK110;
unsigned int get_conflicts(unsigned * combination, unsigned combination_size)
{
	// 预热GPU
	//warmup<<<1, 1>>>();
	// Create input data
	unsigned  *h_combination = 0;  //store a number in [1~N]
	unsigned  *d_combination = 0;
	unsigned * h_result = 0;
	unsigned * d_result = 0;
	// timer 
	...
	//cudaEventRecord(start, 0);
	// allocate GPU mem
	checkCudaErrors(cudaMalloc((void **)&d_combination, combination_size * sizeof(unsigned)));
	checkCudaErrors(cudaMalloc((void **)&d_result ,     1                * sizeof(unsigned)));
	cuda_err = cudaGetLastError();    
	if (cuda_err != cudaSuccess) 
	{                      
	...
	//combination  H->D	
	//cudaEventRecord(start, 0);
	checkCudaErrors(cudaMemcpy(d_combination, h_combination, combination_size * sizeof(unsigned), cudaMemcpyHostToDevice));
	//cudaEventRecord(stop, 0);
	//cudaEventSynchronize(stop);
	//cudaEventElapsedTime(&cuda_time, start, stop);
	...
	h_result[0]=0;
	//cudaEventRecord(start, 0);
	checkCudaErrors(cudaMemcpy(h_result, d_result, 1 * sizeof(unsigned ), cudaMemcpyDeviceToHost));
	cuda_err = cudaSuccess;
	cuda_err = cudaGetLastError();    
	if (cuda_err != cudaSuccess) 
	{                      
