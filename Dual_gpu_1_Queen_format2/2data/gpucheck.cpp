...
int main(int argc, const char *argv[]) 
{
	...
	if(argc==4){
		...
		replication	 = atoi(argv[3]);
		printf("from file: %s  read %d rows. use Device 0 and 1 \n", filename.c_str() , rows_to_read );
	}                                              
	else{
		printf("usage:  check filename row_to_be_read   replicate_of_line\n"); 
		printf("e.g. :  check 4.txt 5   10\n"); 
		exit(0);
    }
	...
	for(int i=0 ;i<rows_already_read; i++){
		...
		for(int t =0; t<replication; t++){
			...
			unsigned int * tmp0 = new unsigned int[ len[i]];
			unsigned int * tmp1 = new unsigned int[ len[i]];
			memcpy(tmp0, a[i], len[i]*sizeof(int) ); // to avoid auto optimaize by sys.
			memcpy(tmp1, a[i], len[i]*sizeof(int) ); // to avoid auto optimaize by sys.
			//reverse_copy(tmp0, tmp0+len[i], tmp1);
			// shuffer it with time-based random seed
			unsigned seed = chrono::system_clock::now ().time_since_epoch ().count ();
			shuffle (tmp1, tmp1+ len[i], default_random_engine (seed));

			c11_start =std::chrono::system_clock::now();

			unsigned int res0 =0;
			unsigned int res1 =0;
			get_conflicts( tmp0, tmp1, len[i], &res0, &res1);
			...
			printf(	"\e[1;31;40m[%10d]  %10d\t%5d/%5d\t%10d\t%10d\e[0m", t, len[i], res0,res1,  milli_seconds,micro_seconds);
			if(res0 ==  conflicts[i]  && res1 == conflicts[i]) 
				printf("\tcpu == GPU0/1, Check Pass!\n");
			else
				printf("\tcpu != GPU0/1, Check False!\n");
			sum_milli += milli_seconds ;
			sum_micro += micro_seconds ;
			sum_conflicts = sum_conflicts +res0 ;
		}
