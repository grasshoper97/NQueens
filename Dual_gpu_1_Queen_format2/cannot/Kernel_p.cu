...
unsigned int get_conflicts(unsigned int * combination, unsigned int combination_size)
{
	...
	h_result0        =(unsigned int *)malloc(	1                * sizeof(unsigned int));
	if(h_result0==NULL )
	{
		printf("malloc h_result0  error \ni");
		exit(1);
	}
	...
	h_result1        =(unsigned int *)malloc(	1                * sizeof(unsigned int));
	if(h_result1==NULL )
	{
		printf("malloc h_result1  error \ni");
		exit(1);
	}
