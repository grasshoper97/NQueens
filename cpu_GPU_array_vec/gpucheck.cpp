...
// 2019.07.02 add compare of vec and array
#include "Kernel.h"
#include <stdio.h>
#include <vector>
#include <string.h>
...
int main(int argc, const char *argv[]) 
{
	...
	if(argc==5){
		...
		printf("from file: %s  read %d rows. use Device %d \n", filename.c_str() , rows_to_read, device);
	}                                              
	else{
	...
	if(rows_to_read > 1)
	{
		printf("this version use vector, row must be 1\n"); 
		exit(0);
    }
	...
	printf("\e[1;31;40m%10s\t%10s\t%10s\t%10s\t%10s\n\e[0m","loop","Queen","conflicts","time(ms)","time(us)");
	printf("%10s\t%10s\t%10s\n","-----","---------","--------");
	for(int i=0 ;i<rows_already_read; i++){
		...
		for(int t =0; t<replication; t++){
			...
			m_conflicts = get_conflicts( tmp, len[i]);
			...
			printf(	"\e[1;31;40m GPU_arr[%2d]  %10d\t%10d\t%10d\t%10d\n\e[0m", t, len[i], m_conflicts, milli_seconds,micro_seconds);
			// .................cpu array........................
			c11_start =std::chrono::system_clock::now();
			m_conflicts = get_conflicts_cpu( tmp, len[i]);
			c11_end = std::chrono::system_clock::now();
	
			delete []tmp;
			milli_seconds = std::chrono::duration_cast<std::chrono::milliseconds>(c11_end-c11_start).count();
			micro_seconds = std::chrono::duration_cast<std::chrono::microseconds>(c11_end-c11_start).count();
			printf(	"\e[1;31;40m cpu_arr[%2d]  %10d\t%10d\t%10d\t%10d\n\e[0m", t, len[i], m_conflicts, milli_seconds,micro_seconds);
			// .................GPU vec........................
			c11_start =std::chrono::system_clock::now();
			m_conflicts = get_conflicts( &(a_vec[0]), a_vec.size() );
			c11_end = std::chrono::system_clock::now();
	
			milli_seconds = std::chrono::duration_cast<std::chrono::milliseconds>(c11_end-c11_start).count();
			micro_seconds = std::chrono::duration_cast<std::chrono::microseconds>(c11_end-c11_start).count();
			printf(	"\e[1;31;40m GPU_vec[%2d]  %10d\t%10d\t%10d\t%10d\n\e[0m", t, len[i], m_conflicts, milli_seconds,micro_seconds);
			// .................cpu vec........................
			c11_start =std::chrono::system_clock::now();
			m_conflicts = get_conflicts_cpu( &(a_vec[0]), a_vec.size() );
			c11_end = std::chrono::system_clock::now();
	
			milli_seconds = std::chrono::duration_cast<std::chrono::milliseconds>(c11_end-c11_start).count();
			micro_seconds = std::chrono::duration_cast<std::chrono::microseconds>(c11_end-c11_start).count();
			printf(	"\e[1;31;40m cpu_vec[%2d]  %10d\t%10d\t%10d\t%10d\n\e[0m", t, len[i], m_conflicts, milli_seconds,micro_seconds);
			// .................over ........................
		}
