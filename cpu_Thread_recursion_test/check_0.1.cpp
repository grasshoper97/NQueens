...
int thread_func(int start, int end, int *sum)
{
		//判断任务是否足够小
		if( end - start < 1 )
		{
		...
		else // 还需要继续分解
		{
			...
			int left 	= 0; 
			int right	= 0;
			t1 = thread( thread_func,  start,    middle,   &left );
			t2 = thread( thread_func,  middle+1, end,      &right);
			//等待子任务执行完，并得到执行结果
			t1.join();
			t2.join();
			*sum = left+ right;
...
int main(int argc, const char *argv[]) 
{
	...
	if(argc==2){
		...
		int result= 0;
		std::thread t = thread ( thread_func, 1, N , &result );
		t.join();
		printf("\n=======================================================================\n");
		printf(" result= %d\n ", result);
